      ...
      <?php endif; ?>
          ...
          <div class="card m-b-100">
            ...
            <input type="hidden" name="id_laundry" value="<?= $id_laundry; ?>">
              ...
              <div class="form-group row">
                <label for="example-text-input" class="col-sm-2 col-form-label">Outlet King Laundry</label>
                <div class="col-sm-10">
                  <!-- jenis() => function javascript -->
                  <!-- onchange => jika dipilih maka fungsi jenis dijalankan -->
                  <select class="select2 form-control mb-3 custom-select" style="width: 100%; height:36px;" name="nama" id="nama" onchange="nama();">
                  <option>--Pilih Outlet---</option>
                  <?php 
                  $query = mysqli_query($conn, "SELECT * FROM outlet");
                  while ($result = mysqli_fetch_assoc($query)) :
                  if ($result['id'] == $jenis) { ?>
                    <option value="<?= $result['id']; ?>" selected><?= $result['nama_outlet']; ?></option>
                  <?php }else{ ?>
                    <option value="<?= $result['id']; ?>"><?= $result['nama_outlet']; ?></option>
                  <?php } ?>
                  <?php endwhile; ?>
                  </select>
                </div>
